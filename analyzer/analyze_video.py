...
def contour_candidates(frame, previous_gray, config, last_point, predicted_point):
    ...
    return gray, motion_energy, candidates
...
def detect_events(track, sample_fps, frame_diagonal, sport):
    events = []
    if len(track) < 5:
        return events
    ...
    for index in range(2, len(track) - 2):
        ...
        if not moving or not event_signal:
            continue
...
def analyze_video(path, sport="tennis", strength=2, max_seconds=900):
    ...
    while capture.isOpened():
        ...
        gray, motion_energy, candidates = contour_candidates(
            frame, previous_gray, config, last_point, predicted
        )
        motion_samples.append({"time": round(timestamp, 3), "energy": round(motion_energy, 5)})
    ...
    capture.release()
    events = detect_events(track, sample_fps, frame_diagonal, sport)
    ...
    tracked_seconds = len(track) / sample_fps
    return {
        "version": 2,
        ...
        "criteria": {
            ...
            "hit": "球轨迹连续时，方向变化或速度变化超过阈值",
            "idle": "持续低运动量且没有可靠球轨迹，达到删除时长阈值",
